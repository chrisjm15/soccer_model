...
# Module 2 — Sofascore Stats + xG Loader
**Target file:** `experiment/load_sofascore.py`
**Model:** Qwen3-Coder 30B
**Run from:** project root `C:\Users\chris\Documents\Claude\Projects\Sports Betting - Soccer`

---

## Context

Write a self-contained Python script `experiment/load_sofascore.py` that fetches per-match stats (including xG) from the Sofascore unofficial API and saves them as CSV files.

The Sofascore API requires a real browser session — plain HTTP requests return 403. The script MUST use Playwright to navigate to sofascore.com first, then make all API calls via `page.evaluate()` (JavaScript fetch inside the browser). This approach works because the browser handles session cookies automatically.

---

## Verified API endpoints (all confirmed working)

```
# Get seasons for a league
GET https://api.sofascore.com/api/v1/unique-tournament/{tournament_id}/seasons

# Get all matches in a round
GET https://api.sofascore.com/api/v1/unique-tournament/{tournament_id}/season/{season_id}/events/round/{round_num}

# Get match statistics (xG lives here)
GET https://api.sofascore.com/api/v1/event/{match_id}/statistics
```
...
### events/round response
```json
{
  "events": [
    {
      "id": 10407050,
      "homeTeam": {"name": "Huddersfield Town"},
      "awayTeam": {"name": "Burnley"},
      "status": {"description": "Ended"},
      "startTimestamp": 1659100800
    }
  ]
}
```
Only process events where `status.description == "Ended"` — skip unplayed matches.

### statistics response — ALL period only
```json
{
  "statistics": [
    {
      ...
      "groups": [
        {
          "statisticsItems": [
            {"key": "expectedGoals",    "homeValue": 0.46, "awayValue": 0.61},
            {"key": "totalShotsOnGoal", "homeValue": 6,    "awayValue": 15},
            {"key": "shotsOnGoal",      "homeValue": 2,    "awayValue": 3},
            {"key": "cornerKicks",      "homeValue": 0,    "awayValue": 5},
            {"key": "fouls",            "homeValue": 6,    "awayValue": 11},
...
**IMPORTANT stat key notes:**
- `totalShotsOnGoal` = TOTAL shots (despite the misleading name) → maps to home_shots/away_shots
- `shotsOnGoal` = shots on target → maps to home_sot/away_sot
- `redCards` may be absent entirely if no red cards in the match — use `.get()` with default 0
- Always use the `ALL` period (not `1ST` or `2ND`)
- Use `homeValue`/`awayValue` (numeric) not `home`/`away` (string) for stat values

---

## League IDs (uniqueTournament IDs, verified)
...
Season type notes:
- `european`: season name is "25/26" format → convert to "2024-2025" for our file names
- `calendar`: season name is "2026" format → keep as "2026" for file names
- `danish`: season name is "25/26" format → convert same as european

---

## Season name conversion
...
One CSV file per league per season, saved to:
- `data/proxy/{league}_{season}_sofascore.csv`     for european/danish leagues
- `data/scandinavian/{league}_{season}_sofascore.csv` for calendar leagues
...
- `date`: YYYY-MM-DD string, converted from Unix timestamp using `pd.to_datetime(ts, unit='s').strftime('%Y-%m-%d')`
- `home_xg`/`away_xg`: float (2 decimal places), all other numeric cols: int
- Missing stats: None (not 0) — use `None` as default, not 0
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for league_code in league_list:
            ...
            output_dir = 'data/scandinavian' if league_type == 'calendar' else 'data/proxy'
...
## Known Qwen3-Coder bugs to guard against

1. **Do NOT use `pd.compat.StringIO`** — use `import io; io.StringIO` instead
2. **Column rename BEFORE reindex** — if renaming columns, do it before `df.reindex(columns=OUTPUT_COLUMNS)`
3. **No module-level side effects** — all directory creation and browser setup inside `main()`
4. **Date format: always use `.strftime('%Y-%m-%d')`** — never return a raw timestamp or raw string
5. **Use `.get()` for ALL dict access on API responses** — never use `[]`
6. **`redCards` key may be absent** — always `.get('redCards')` with default None, never assume it exists
...
```bash
# Scrape current season for all 10 leagues (run today)
python experiment/load_sofascore.py --current-only

# Scrape specific leagues, current season
python experiment/load_sofascore.py --leagues E1 N1 --current-only

# Scrape all seasons (run overnight with laptop plugged in)
python experiment/load_sofascore.py --all-seasons
```

A browser window will open automatically. Do not close it while the script runs.

---

## Output verification (Colab QC)

After running, check the output CSV with:
```python
import pandas as pd
df = pd.read_csv('data/proxy/E1_2024-2025_sofascore.csv')
print(df.shape)
print(df[['home_team','away_team','home_xg','away_xg','home_shots','away_shots']].head(10))
print(df[['home_xg','away_xg']].describe())
print(f"xG nulls: {df['home_xg'].isna().sum()} / {len(df)}")
```

Expected: ~400-550 rows for Championship, xG null rate under 10%.
